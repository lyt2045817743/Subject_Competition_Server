...
router.post('/addTeam', async ctx => {
    ...
    const { teamName, managerId, membersId, teacherId, teamState, joinCondition } = ctx.request.body;
    const team = new Team({
        teamName,
        managerId,
        membersId,
...
router.get('/queryTeamList', async ctx => {
    ...
    if(pageNum == -1 && pageCount == -1) {
        listdata = await Team.find({"teamName" : {$regex: keyword ? keyword : ''}})
        .sort({'createTime': -1})
...
// // 根据teamName获取单个团队信息
// router.get('/getTeamInfo/:teamName', async ctx => {
//     const teamName = ctx.params.teamName;
//     await Team.findOne({teamName}).then( res => {
